...
import { getAccessToken, getThirdpartyDevice, thirdpartyDevice, updateThirdpartyDevice } from '../services/ihost.ts'
...
thirdpartyRouter.post('/open-api/device/:deviceId', async (req, res, next) => {
  try {
    ...
    if (header.name === 'UpdateDeviceStates') {
      ...
      const params = buildEndpoint[device[0].model as keyof typeof buildEndpoint].stateToParams(payload.state);
